...
class JMI():
    """ Class for JMI features selection
    ...
    def __init__(self, num_of_features=20):
        if sys.platform.startswith('linux'):
            ...
            raise NotImplementedError('FeastJMI is not supported on {} operating system'.format(sys.platform))
    ...
    def fit(self, data, labels):
        """
        Fits a defined JMI filter to a given data set.
        ...
        self.libFSToolbox.JMI.argtypes = [c.c_uint, c.c_uint, c.c_uint, _uintpp, c.POINTER(c.c_uint),
                                           c.POINTER(c.c_uint), c.POINTER(c.c_double)]
        self.libFSToolbox.JMI.restype = c.POINTER(c.c_uint)

        # call the C lib implementation
        c_selected_features = self.libFSToolbox.JMI(
