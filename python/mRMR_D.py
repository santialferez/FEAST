...
class mRMR_D():
    """ Class for mRMR_D features selection
    ...
    def __init__(self, num_of_features=20):
        if sys.platform.startswith('linux'):
            ...
            self.libFSToolbox = c.CDLL("../libFSToolbox.so")
            ...
            raise NotImplementedError('FeastmRMR_D is not supported on {} operating system'.format(sys.platform))
    ...
    def fit(self, data, labels):
        """
        Fits a defined mRMR_D filter to a given data set.
        ...
        self.libFSToolbox.mRMR_D.argtypes = [c.c_uint, c.c_uint, c.c_uint, _uintpp, c.POINTER(c.c_uint),
                                           c.POINTER(c.c_uint), c.POINTER(c.c_double)]
        self.libFSToolbox.mRMR_D.restype = c.POINTER(c.c_uint)

        # call the C lib implementation
        c_selected_features = self.libFSToolbox.mRMR_D(
